...
Arguments: the text you type after the command — parsed exactly like `/jira-issue` (see `jira-issue.md`): breadcrumb first, optional `model:`/`effort:` tokens anywhere, the rest is the description. The filing flow is `/jira-issue`'s, unchanged. The deep-dive phases and ALL their ground rules come from the two siblings and apply verbatim here:
