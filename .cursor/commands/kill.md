...
Arguments: the text you type after the command — empty means **stabilize automatically** (kill newest-first until pressure recovers); a number kills exactly that many newest agents; `all` stops every running sub-agent. This is an emergency command: act immediately, measure, report after — do not ask for confirmation mid-incident.
