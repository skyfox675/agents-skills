...
- `.claude/agents/<agent-name>.md` — one file per agent; frontmatter (`name`, `description`, `tools`, optional `model`) + system prompt. Ships 12 tiered worker agents (`scout` on the cheap tier; `builder`, `reviewer`, `implementer`, `pr-rescuer`, `diagnostician`, `verifier`, `spelunker`, `design-mapper` on workhorse — these the commands dispatch — plus the `pr-comments`, `pr-checks`, `pr-cleanup` single-lane PR watchers run via `/loop`). Canonical agents live here; `make sync-agents` generates the Cursor (`.cursor/agents/`, `model`/`readonly` schema) and Copilot (`.github/agents/*.agent.md`) copies — never hand-edit those. Keep agents thin and point at the relevant skill for the protocol.
