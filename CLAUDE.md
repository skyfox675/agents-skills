...
- `.claude/agents/<agent-name>.md` — one file per agent; frontmatter (`name`, `description`, `tools`, optional `model`) + system prompt. Ships 9 tiered worker agents the commands dispatch (`scout` on the cheap tier; `builder`, `reviewer`, `implementer`, `pr-rescuer`, `diagnostician`, `verifier`, `spelunker`, `design-mapper` on workhorse). Canonical agents live here; `make sync-agents` generates the Cursor (`.cursor/agents/`, `model`/`readonly` schema) and Copilot (`.github/agents/*.agent.md`) copies — never hand-edit those. Keep agents thin and point at the relevant skill for the protocol.
